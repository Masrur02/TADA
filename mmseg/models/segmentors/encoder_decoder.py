...
class EncoderDecoder(BaseSegmentor):
    ...
    def generate_pseudo_label(self, img, img_metas):
        ...
        out,trav = self.encode_decode(img, img_metas)
        if self.debug:
    ...
    def encode_decode(self, img, img_metas, upscale_pred=True):
        ...
        out,trav = self._decode_head_forward_test(x, img_metas)
        if upscale_pred:
            out = resize(
                input=out,
                size=img.shape[2:],
    ...
    def _decode_head_forward_train(self,
        ...
        training."""
        losses = dict()
        loss_decode,seg_logits,trav,trav_gt = self.decode_head.forward_train(x, img_metas,
                                                     gt_semantic_seg,
                                                     self.train_cfg,
                                                     seg_weight, return_logits)

        losses.update(add_prefix(loss_decode, 'decode'))
        return losses,seg_logits, trav,trav_gt

    def _decode_head_forward_test(self, x, img_metas):
        """Run forward function and calculate loss for decode head in
        inference."""
        seg_logits,trav = self.decode_head.forward_test(x, img_metas, self.test_cfg)
        return seg_logits,trav
    ...
    def forward_train(self,
        ...
        loss_decode,seg_logits,trav,trav_gt = self._decode_head_forward_train(x, img_metas,
                                                      gt_semantic_seg,
                                                      seg_weight,
                                                      return_logits)
    ...
    def slide_inference(self, img, img_meta, rescale):
        ...
        if batched_slide:
            ...
            crop_seg_logits,trav = self.encode_decode(crop_imgs, img_meta)
            ...
            for h_idx in range(h_grids):
                for w_idx in range(w_grids):
                    ...
                    x1 = max(x2 - w_crop, 0)
                    crop_img = img[:, :, y1:y2, x1:x2]
                    crop_seg_logit,trav = self.encode_decode(crop_img, img_meta)
                    preds += F.pad(crop_seg_logit,
                                   (int(x1), int(preds.shape[3] - x2), int(y1),
                                    int(preds.shape[2] - y2)))
    ...
    def whole_inference(self, img, img_meta, rescale):
        ...
        seg_logit,trav = self.encode_decode(img, img_meta)
        if rescale:
    ...
    def inference(self, img, img_meta, rescale):
        ...
        if self.test_cfg.mode == 'slide':
            seg_logit,trav = self.slide_inference(img, img_meta, rescale)
        else:
            seg_logit,trav = self.whole_inference(img, img_meta, rescale)
    ...
    def simple_test(self, img, img_meta, rescale=True):
        ...
        seg_logit,trav = self.inference(img, img_meta, rescale)
        if hasattr(self.decode_head, 'debug_output_attention') and \
                self.decode_head.debug_output_attention:
    ...
    def aug_test(self, imgs, img_metas, rescale=True):
        ...
        seg_logit,trav = self.inference(imgs[0], img_metas[0], rescale)
